...
st.markdown(
    ...
    [data-testid="collapsedControl"] {
        ...
        box-shadow: 0 0 0 3px rgba(56,189,248,0.35), 0 2px 8px rgba(0,0,0,0.28) !important;
    ...
    @keyframes pulse-sidebar {
        0%   { box-shadow: 0 0 0 3px rgba(56,189,248,0.45), 0 2px 8px rgba(0,0,0,0.28); }
        50%  { box-shadow: 0 0 0 7px rgba(56,189,248,0.12), 0 2px 8px rgba(0,0,0,0.28); }
        100% { box-shadow: 0 0 0 3px rgba(56,189,248,0.45), 0 2px 8px rgba(0,0,0,0.28); }
    }
    </style>
    """,
...
with st.sidebar:
    ...
    with st.expander("ℹ️ About", expanded=True):
        st.markdown(
            ...
            **Construction Scheduler** is an AI-powered project scheduling tool
            ...
            The intention of the app is not to replace the expertise of a project scheduler, 
            but to accelerate the process of schedule generation and provide a starting point for further refinement.
    ...
    with st.expander("✨ Features — v0.1.1", expanded=False):
        st.markdown(
            """
            **v0.1.1 — 11-Jul-2026**
            - 🔒 Generate Schedule button locked until AI activities are generated, reviewed & saved
            - ⚡ API rate limiter (RPM / TPM) to prevent 429 errors
            - 🚦 Clear error messages for quota exhaustion, rate limits & missing API key
            - 🔵 Prominent sidebar toggle button with pulse animation
            """
        )

    with st.expander("📋 Previous Releases", expanded=False):
        st.markdown(
            """
            **v0.1.0 — 5-Jul-2026**
        ...
        with st.spinner("Calling AI — this may take 15–30 seconds…"):
            try:
                ...
                if "insufficient_quota" in err_str or "exceeded your current quota" in err_str:
                    ...
                    st.error(
                        "**Rate limit hit (429).** Too many requests in a short window.\n\n"
                        "Wait a minute, then try again. You can also lower the request rate by setting "
                        "`OPENAI_RPM` / `OPENAI_TPM` in your `.env` file to match your plan limits."
                    )
                elif "OPENAI_API_KEY" in err_str:
                    st.error("**API key missing.** Set `OPENAI_API_KEY` in your `.env` file.")
                else:
...
if st.session_state.ai_activities_draft is not None:
    ...
    _activities_ready = st.session_state.ai_activities is not None
    if not _activities_ready:
        st.warning("⚠️ No activities found. Please generate and save activities in **Section 4** before generating the schedule.")
    if st.button("▶ Generate Schedule", key="generate_schedule", disabled=not _activities_ready):
