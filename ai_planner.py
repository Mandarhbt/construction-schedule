...
# ── Rate Limiter ───────────────────────────────────────────────────────────────
# Limits are read from environment variables so you can match your API tier:
#   OPENAI_RPM  – max requests per minute  (default: 500)
#   OPENAI_TPM  – max tokens per minute    (default: 30000)
# Check your actual limits at: https://platform.openai.com/account/rate-limits

class _SlidingWindowRateLimiter:
    """Thread-safe sliding-window rate limiter for OpenAI API calls.

    Tracks requests and token usage over the last 60 seconds and blocks
    (sleeps) until a call would be within the configured RPM/TPM limits.
    """
    ...
    def __init__(self, rpm: int, tpm: int):
        ...
        self._req_times: deque = deque()          # timestamps of recent requests
        self._token_log: deque = deque()          # (timestamp, tokens) pairs

    def _evict(self, now: float) -> None:
        """Drop entries older than 60 s (outside the sliding window)."""
        cutoff = now - 60.0
    ...
    def acquire(self, estimated_tokens: int = 0) -> None:
        """Block until the next request would stay within RPM and TPM limits."""
        while True:
            with self._lock:
                now = time.time()
                self._evict(now)
                ...
                tokens_ok = (used_tokens + estimated_tokens) <= self.tpm
                if reqs < self.rpm and tokens_ok:
                    self._req_times.append(now)
                    self._token_log.append((now, estimated_tokens))
                    return   # cleared to proceed
            # Rate limit would be exceeded — wait 1 s then re-check
            wait_reason = "RPM" if len(self._req_times) >= self.rpm else "TPM"
            logger.warning(
                "Rate limit (%s) reached. Waiting 1 s before retrying API call …", wait_reason
            )
            time.sleep(1.0)


def _make_rate_limiter() -> _SlidingWindowRateLimiter:
    try:
        rpm = int(os.environ.get("OPENAI_RPM", "500"))
    except ValueError:
        rpm = 500
    try:
        tpm = int(os.environ.get("OPENAI_TPM", "30000"))
    except ValueError:
        tpm = 30000
    return _SlidingWindowRateLimiter(rpm=rpm, tpm=tpm)


_rate_limiter: _SlidingWindowRateLimiter = _make_rate_limiter()
...
def call_llm(prompt: str, model: str = "gpt-4o", temperature: float = 0.0, max_tokens: int = 4096) -> str:
    ...
    A sliding-window rate limiter (see _rate_limiter) prevents HTTP 429 errors
    by honouring the OPENAI_RPM / OPENAI_TPM limits you set in your environment.
    """
    if openai is None:
    ...
    # Estimate tokens: ~4 chars ≈ 1 token for the prompt + max_tokens for the reply
    estimated_tokens = len(prompt) // 4 + max_tokens
    _rate_limiter.acquire(estimated_tokens=estimated_tokens)
